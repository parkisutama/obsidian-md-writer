# Obsidian MD Writer — Project Overview

**MD Writer** is an Obsidian community plugin that provides a specialized environment for high-precision drafting. It combines typewriter scrolling, whitespace visualization, and outliner zoom focus.

- **Plugin ID**: `md-writer`
- **Current version**: see `manifest.json`
- **Minimum Obsidian**: v1.0.0
- **Author**: Parkis Utama
...
│   ├── features/                  # 12 feature categories (typewriter, dimming, currentLine, …)
│   ├── commands/                  # 10 commands (toggles, move typewriter up/down, writing focus, zoom)
...
│   ├── selectors.ts
│   ├── per-window-props.ts
│   └── zoom/                      # Zoom/outliner focus extensions
├── components/
...
| Script | Description |
| -------- | ------------- |
| `bun run dev` | Build + set up test vault (main dev workflow) |
| `bun run build` | Build plugin only |
| `bun run debug` | Build with debug statements enabled |
| `bun run typecheck` | TypeScript type check |
| `bun run lint` | Biome lint & format |
| `bun run lint:styles` | SCSS linting |
| `bun run lint:md` | Markdown linting |
| `bun run check` | All of the above |
| `bun run release` | Run checks then release (creates tag + GitHub release) |
